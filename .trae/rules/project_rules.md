...
- 代码变更后必须在本地执行：
  ...
  - `npm run lint:all`（ESLint/Prettier/Stylelint 全量检查）
- 页面功能验证：运行开发服务并进行实际路径访问与交互测试。
- 路由/菜单验证点：
  - 动态路由收集：`src/router/routes/index.ts:8`（`import.meta.glob`）
  - 路由注册：`src/store/modules/user.ts:168-176`（登录后动态添加）
  - 菜单过滤与构建：`src/router/menus/index.ts` 与 `src/store/modules/permission.ts`
