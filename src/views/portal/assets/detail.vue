<template>
  <div class="p-4">
    <Description :title="t('routes.portal.assetDetail')" :schema="descSchema" :data="detailData" bordered />
...
<script setup lang="ts">
  ...
  async function loadDetail() {
    const id = String(route.params.assetId);
    const res = await findEntityDataByQuery({
      entityFilter: { type: 'singleEntity', singleEntity: { id, entityType: EntityType.ASSET } },
      entityFields: [
      ...
      latestValues: ASSET_FIELDS.order.map((k) => ({ type: 'SERVER_ATTRIBUTE', key: k })),
