    ...
    <Form ref="formRef" :model="formState" layout="vertical">
      ...
      <Form.Item name="sparkplug">
        ...
        <p class="text-xs text-neutral-500" v-show="formState.sparkplug == true"
          >允许来自具备Sparkplug B负载和Topic格式的边缘网络（EoN）节点的连接。</p
        >
      </Form.Item>

      <Form.Item
        label="将SparkPlug指标存储为属性。"
      ...
      >
        ...
        <p class="text-xs text-neutral-500"
          >将作为设备属性存储的SparkPlug指标的名称。其他指标将作为设备遥测数据进行存储。</p
        >
      </Form.Item>

      <CollapseContainer
        title="MQTT 设备 Topic 筛选器"
        :canExpan="false"
        ...
        <template v-if="formState.transportPayloadTypeConfiguration.transportPayloadType == 'PROTOBUF'">
          ...
          >
            ...
            <p class="text-xs text-neutral-500">
              ...
              /req/$request_id，v2/r/res/$request_id。其中$request_id是一个整数请求标识符。
            </p>
          </Form.Item>
          <Form.Item
      ...
      <Form.Item name="sendAckOnValidationException" v-show="formState.sparkplug == false">
        ...
        <p class="text-xs text-neutral-500"
          >默认情况下平台将关闭相关消息验证失败的MQTT会话，启用后平台将发布确认而不是关闭会话。</p
        >
      </Form.Item>
    </Form>
  </div>
